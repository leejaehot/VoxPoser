...
from pyrep.const import ObjectType # 여기서 object id를 어케어케하는 것 같긴한데,
...
class VoxPoserRLBench():
    def __init__(self, visualizer=None):
        ...
        self.camera_names = ['front', 'left_shoulder', 'right_shoulder', 'overhead', 'wrist'] # 카메라가 5개.
    ...
    def get_object_names(self):
        ...
        """
        name_mapping = self.task_object_names[self.task.get_name()] # bottle, bottle, cap, cap
        exposed_names = [names[0] for names in name_mapping] # bottle, cap
    ...
    def load_task(self, task):
        ...
        # scene_objs = env.task._task.get_base().get_objects_in_tree(object_type=ObjectType.SHAPE, exclude_base=False, first_generation_only=False)
    ...
    def get_3d_obs_by_name(self, query_name): # 3D pc observation by obj name
        """
        ...
        obj_ids = self.name2ids[query_name] # 이놈 self.name2ids는 또 어떻게 구해지는 거지?
        # gather points and masks from all cameras
        points, masks, normals = [], [], []
        depths, rgbs = [], []
        for cam in self.camera_names: # 다 128,128,3
            points.append(getattr(self.latest_obs, f"{cam}_point_cloud").reshape(-1, 3)) # (16384, 3)
            masks.append(getattr(self.latest_obs, f"{cam}_mask").reshape(-1)) # (49512, ) 아니 이거 어떻게 구해지는 거지?
            
            depths.append(getattr(self.latest_obs, f"{cam}_depth")) # custom
            rgbs.append(getattr(self.latest_obs, f"{cam}_rgb")) # custom
            # 여기서 이제 어떻게 더 할까?
            # env.latest_obs에서 misc활용 -> extrinsic, intrinsic
            # 저 GT PC와 실제 depth map - camera extrinsic - 지금 어디 좌표계기준으로 향해있는가.
            # 저 GT PC를 구하는 과정의 함수를 뜯어보자. or GaussianGrasper 논문 함수 뜯어보라.
